...
void formatFileName(const char *input, char *output)
{
    ...
    const char *nameEnd = dot ? dot : input + strlen(input);
    const char *extStart = dot ? dot + 1 : NULL;

    // Copy name (up to 8 characters)
    size_t nameLength = nameEnd - input;
    if (nameLength > 8)
        nameLength = 8;
    memcpy(output, input, nameLength);

    // Copy extension (up to 3 characters)
    if (extStart)
    {
        size_t extLength = strlen(extStart);
        if (extLength > 3)
            extLength = 3;
        memcpy(output + 8, extStart, extLength);
    }
}
