...
ROOT_DIR_SECTORS        equ 14                      ; Root directory size in sectors
...
start:
    ...
    ; Ensure we are at the correct memory location
    push es
    push word .after
    retf
...
.after:
    ...
    mov cl, ROOT_DIR_SECTORS            ; CL = number of sectors to read
...
cluster_to_lba:
    ...
    add ax, [bdb_reserved_sectors]org 0x7C00
bits 16
...
ebr_signature:              db 29h
ebr_volume_id:              db 12h, 34h, 56h, 78h   ; serial number, value doesn't matter
ebr_volume_label:           db 'Dead       '        ; 11 bytes, padded with spaces
ebr_system_id:              db 'FAT12   '           ; 8 bytes
...
;
; Gets next cluster in FAT chain
; Params:
;   - ax: current cluster
; Returns:
;   - ax: next cluster
